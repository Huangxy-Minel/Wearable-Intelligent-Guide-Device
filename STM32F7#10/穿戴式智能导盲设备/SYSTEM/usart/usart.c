...
{	
	...
	HAL_UART_Init(&UART4_Handler);					    //HAL_UART_Init()会使能UART5
...
//		__HAL_UART_DISABLE_IT(huart,UART_IT_TC);
	...
	if(huart->Instance==UART4)//如果是串口5，进行串口5 MSP初始化
	{
		__HAL_RCC_GPIOA_CLK_ENABLE();			//使能GPIOA时钟
		...
		GPIO_Initure.Alternate=GPIO_AF6_UART4;	//复用为UART5
		HAL_GPIO_Init(GPIOA,&GPIO_Initure);	   	//初始化PB12

		GPIO_Initure.Pin=GPIO_PIN_12;			//PB13
		HAL_GPIO_Init(GPIOA,&GPIO_Initure);	   	//初始化PB13
		
#if EN_UART4_RX
		HAL_NVIC_EnableIRQ(UART4_IRQn);				//使能UART5中断通道
	...
	if(huart->Instance==UART5)//如果是串口5，进行串口5 MSP初始化
	{
		...
		HAL_GPIO_Init(GPIOB,&GPIO_Initure);	   	//初始化PB13
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	if(huart->Instance==UART5)//如果是串口5
	{
		...
		{
			if(UART5_COUNT == 100 && aRxBuffer[0] == 0x55)//还未开始接收帧的时候发现帧头
			{
				UART5_COUNT = 55;//发现第一个帧头标志位
				UART5_RX_STA = 0;
				UART5_RX_BUF[UART5_RX_STA&0X3FFF]=aRxBuffer[0];
				UART5_RX_STA++;
			}
			else if(UART5_COUNT == 55 && aRxBuffer[0] == 0x51)//继第一个帧头后发现第一个功能帧头
			{
				UART5_RX_BUF[UART5_RX_STA&0X3FFF]=aRxBuffer[0];
				UART5_RX_STA++;
				UART5_COUNT = 0;
			...
			else if(UART5_START == 1)//启动接收
			{
				UART5_RX_BUF[UART5_RX_STA&0X3FFF]=aRxBuffer[0];
				UART5_RX_STA++;
				UART5_COUNT+=1;
				if(UART5_COUNT == 42)//已经接受了四十二个数据
				{
					//标记标志位，恢复接收中断使用的值
					UART5_RX_STA|=0x8000;
					UART5_COUNT = 100;
