...
{ 	  
    ...
    GPIO_Initure.Pin=GPIO_PIN_11;            //PB5,背光控制
...
//波形绘制函数：points-点的个数， *points_array-数组地址，x-y轴横坐标移位量，y-x轴纵坐标移位量
void Draw_Wave(u16 points,float *points_array,u16 x,u16 y)
{
	u16 i;
	for(i=0;i<points;i++)
		{
				LCD_DrawLine(i+x,y+points_array[i]/1000,i+1+x,y+points_array[i+1]/1000);
		}
}
...
{  
  ...
  if(Dir==0){  
    ...
    LCD_WriteReg(0x0003,0x1030);              //图像显示方向为左下起  行递增  列递增  
    LCD_WriteReg(0x0210, StartX);             //水平显示区起始地址 0-239  
    LCD_WriteReg(0x0211, StartX+x);               //水平显示区结束地址 0-239  
    LCD_WriteReg(0x0212, StartY);             //垂直显示区起始地址 0-399  
    LCD_WriteReg(0x0213, StartY+y);                   //垂直显示区结束地址 0-399  
    LCD_WriteReg(0x0200, StartX);                 //水平显示区地址  
    LCD_WriteReg(0x0201, StartY);                 //垂直显示区地址  
  }    

  else if(Dir==1){  
    LCD_WriteReg(0x0003,0x1018);              //图像显示方向为左下起  行递增  列递减  
    LCD_WriteReg(0x0210, StartY);             //水平显示区起始地址 0-239  
    LCD_WriteReg(0x0211, StartY+y);           //水平显示区结束地址 0-239  
    LCD_WriteReg(0x0212, 799-(x+StartX));     //垂直显示区起始地址 0-399  
    LCD_WriteReg(0x0213, 799-StartX);         //垂直显示区结束地址 0-399
    LCD_WriteReg(0x200, StartY);                  //水平显示区地址  
    LCD_WriteReg(0x201, 799-StartX);              //垂直显示区地址  
  }    
...
//image2lcd设置为水平扫描，形参：StartX、ENDX是横坐标起始&终止值、StartY是纵坐标起始值，ENDY是截Y宽度的值
...
//				LCD_WriteRAM_Prepare();
				total = (EndX-StartX+1) * EndY;
				for(i=0;i<total;i++)
					{
						LCD_Fast_DrawPoint(x,y,*pic++);
						x++;
						if(x>=EndX)
							{
								y++;
